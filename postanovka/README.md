...
Первый раздел артефакта в обоих профилях, сразу после `Status:`. 3–7 строк: за десять
секунд читатель узнаёт, что меняется, где, какие значения ключевые и зачем.

**Тест читателя.** Каждая строка читается так, будто тела постановки нет. Остался
вопрос «что именно?», «что с этим происходит?» или «и что это даёт?» — строка
переписывается.

Было — опись предметов, тест не проходит:

```markdown
* **Что:** строка оценки и отзывов, однострочные чипы фасовок и минус на кнопках покупки.
* **Где:** карточка товара в списках; минус — также на странице товара.
* **Текст:** причина сниженной цены — «Скидка из-за срока годности» или «Уценка».
* **Платформы:** десктоп и мобильная версия веба.
* **Зачем:** лишние элементы в карточке, убавить количество можно только в корзине.
* **Не входит:** фильтры и сортировка, страница корзины, мобильное приложение.
```

Стало — изменения глаголом, назначение вместо детали, значения в контексте, новая иконка
не потеряна:

```markdown
* **Что:** добавляется строка **оценки и отзывов**, изменяются чипы фасовок, добавляется **управление количеством** товара в корзине через каталог — сейчас убавить количество или убрать товар можно только из корзины.
* **Где:** основные изменения — в каталоге товаров на десктопе и в мобильной версии веба; на **странице товара** меняются только каунтер и иконка скидки в чипе фасовки.
* **Текст:** изменяется подпись причины сниженной цены — **«Скидка из-за срока годности»** или **«Уценка»**.
* **Иконка:** для скидки добавляется **новая иконка**, а огонь остаётся только для уценки, истекающего срока годности и подобных причин.
* **Зачем:** упростить карточку — убрать лишние элементы: плашку и сердечко на чипе фасовки, подписи со сроком доставки под кнопками покупки.
* **Не входит:** страница корзины, мобильное приложение.
```

| Правило | Суть |
| --- | --- |
| Сначала изменение | каждое изменение начинается с глагола: добавляется, изменяется, убирается, заменяется, переносится |
| Назначение, а не деталь | «управление количеством (каунтер)», а не «минус на кнопках покупки»; «однострочный» и «многоточие» остаются в требованиях |
| Текущее ограничение | коротким противопоставлением у того изменения, которое его снимает: «— сейчас … только из корзины» |
| Значения после сказанного | «изменяется подпись причины сниженной цены — «Уценка»», а не голые значения |
| Полнота | каждое видимое изменение из требований узнаётся в блоке; пропущенная новая иконка — дефект блока |
| Недостающее значение | называется прямо: «место в списке блока не названо (см. вопрос 1)» |

Слоты — меню, а не комплект: «Что», «Где», объект изменения («Текст», «Иконка»,
«Адрес»), «Платформы» — только если различаются между изменениями, «Зачем», «Не
входит». Блок — **проекция, а не источник**: каждый его факт присутствует ниже.
Пишется последним, когда тело постановки готово. Чисел-целей в нём нет — метрика
живёт только в «Цели».
...
| Кратко | да | да | 3–7 строк: изменения глаголом, где, ключевые значения, зачем |
