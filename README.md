...
Первым разделом в обоих профилях идёт блок **«Кратко»** — 3–7 строк, по которым
задача читается за десять секунд: что меняется (глаголом — «добавляется»,
«изменяется», «убирается»), где, какие значения ключевые и зачем. Каждая строка
понятна без тела постановки. Точные значения в требованиях и критериях выделяются
жирным.
